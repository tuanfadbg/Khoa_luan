...
folder = "model_neural_network_feed_forward"
...
for i in range(4):
    if i != 1:
        ...
        for j in range(len(data[i])):
            tf_idf_vector.append(np.array(encoder.encode(data[i][j])))
        ...
        for k in range(len(categories[i][0])):
            ...
            history = model.fit(
                ...
                batch_size=64,
                epochs=epochs,
                validation_data=(X_test, y_test_np[:, k]),
                validation_steps=30,
            ...
            loss, acc = model.evaluate(X_test, y_test_np[:, k], verbose=2)
